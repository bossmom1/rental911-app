...
import { saveDocumentMetadata } from '@/app/(landlord)/landlord/(portal)/documents/actions';
import { useRouter } from 'next/navigation';

const DOC_TYPES = [
  { key: 'lease',               label: 'Lease Agreement' },
  { key: 'hoa_governing',       label: 'HOA Governing Docs / CC&Rs' },
  { key: 'hoa_violation',       label: 'HOA Violation Notice' },
  { key: 'management_contract', label: 'Prior Management Contract' },
...
export function DocumentUpload({
  ...
  async function handleUpload(e: React.FormEvent) {
    ...
    const filePath = `${landlordId}/${type}/${Date.now()}-${file.name}`;
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <div>
          ...
          <input
            ...
            placeholder="e.g. 123 Main St Lease 2024"
            className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm"
          />
        </div>

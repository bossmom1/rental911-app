...
export default async function LeaseDetail({ params }: { params: { leaseId: string } }) {
  ...
  const [{ data: renewal }, { data: moveOutChecklist }] = await Promise.all([
    supabase
      .from('lease_renewals')
      .select('*')
      ...
      <div className="mb-6">
        <Card>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <p>
              <span className="text-ink/60">Security deposit:</span> {fmtMoney(lease.security_deposit)}
            </p>
            <p>
          ...
          {lease.is_month_to_month && (
            <p className="mt-3 text-ink/70">
              Month-to-month{lease.month_to_month_note ? ` — ${lease.month_to_month_note}` : ''}
            </p>
          )}
        </Card>
      </div>
